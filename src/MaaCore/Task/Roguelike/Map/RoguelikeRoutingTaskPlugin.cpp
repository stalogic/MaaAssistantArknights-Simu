    ...
    case RoutingStrategy::JieGarden_FastPassWithBattle:
        ...
        if (m_map.get_curr_pos() == RoguelikeMap::INIT_INDEX) {
            ...
            if (m_map.get_node_cost(next_node) >= 30) {
                ...
                TrajectoryLogger::instance().log_generic(ctrler()->get_image(), "routing", json::object{{"node_index",static_cast<int>(next_node)}}.to_string(), "route node=" + std::to_string(next_node), false, "");
                sleep(200);
